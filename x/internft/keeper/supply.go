...
func (k Keeper) iterateTraitsOfClass(ctx sdk.Context, classID string, fn func(trait internft.Trait)) {
	prefix := traitKeyPrefixOfClass(classID)

	k.iterateImpl(ctx, prefix, func(_, value []byte) {
		var trait internft.Trait
		k.cdc.MustUnmarshal(value, &trait)
...
func (k Keeper) iteratePropertiesOfClass(ctx sdk.Context, nft internft.NFT, fn func(property internft.Property)) {
	prefix := propertyKeyPrefixOfNFT(nft.ClassId, nft.Id)
